...
class EpisodeScraper {
    ...
    async scrape(episode, previous) {
        ...
        $infobox.find('td').each(function (i) {
            let $td = $(this);
            if(i === 0) ret.season = parseInt($td.text().trim().substr(7));
            if(i === 1) ret.episode = parseInt($td.text().trim().substr(8));
        ...
        $('#First, #Characters').parent().next().find('li').each(function () {
            ...
            if($a.text().trim().length > 0) ret.characters.push($a.text().trim().replace('\n', ''));
        });
        ...
        $('#Deaths').parent().next().find('li').each(function () {
            let $li = $(this);
            let $a = $li.find('a').first();
            if($a.text().trim().length > 0) ret.deaths.push($a.text().trim().replace('\n', ''));
        });

        // parsing places
        $('#Summary, #Synopsis, #Plot').parent().nextUntil('h2').each(function () {
            let $this = $(this);
            if($this[0]['name'] !== 'h3') return true;
