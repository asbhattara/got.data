...
class HouseScraper {
    ...
    async getAllNames() {
        ...
        for (let i = 0; i < totalhits; i = i + 10) {
            ...
            for(let j = 0; j < data.query.search.length; j++) {
                ...
                houses.push({"name": title, "slug": title});
            }
    ...
    async scrapeAll() {
        ...
        for(let i = 0; i < houses.length; i++)
        {
            ...
            console.log("scraping", house["name"], "(", (i + 1), "/", houses.length, ")");

            data.push(await this.scrape(house["name"], house["slug"]));
        }
